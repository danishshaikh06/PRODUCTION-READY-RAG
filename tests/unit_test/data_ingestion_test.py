...
import pytest
from my_rag_app.core.ingestion.data_ingestion import ThreadResolver, MessageParser
...
class TestThreadResolver:
    ...
    def test_references_header_resolves_to_shared_root(self):
        emails = [
            {"id": "<root@x.com>", "subject": "Slot Request", "from": ["a@x.com"], "to": ["b@x.com"],
             "date": None, "_references": [], "_in_reply_to": ""},
            {"id": "<reply1@x.com>", "subject": "RE: Slot Request", "from": ["b@x.com"], "to": ["a@x.com"],
             "date": None, "_references": ["<root@x.com>"], "_in_reply_to": "<root@x.com>"},
            {"id": "<reply2@x.com>", "subject": "RE: RE: Slot Request", "from": ["a@x.com"], "to": ["b@x.com"],
             "date": None, "_references": ["<root@x.com>", "<reply1@x.com>"], "_in_reply_to": "<reply1@x.com>"},
        ]
    ...
    def test_subject_fallback_links_headerless_thread(self):
        ...
        emails = [
            {"id": "<orig@smb-freight.com>", "subject": "NOC from MoCA to operate cargo flight at VAOZ airport",
             "from": ["test@smb-freight.com"], "to": ["test.moca@nic.in"], "date": "2026-06-03T11:49:00",
             "_references": [], "_in_reply_to": ""},
            {"id": "<reply@smb-freight.com>", "subject": "RE: NOC from MoCA to operate cargo flight at VAOZ airport",
             "from": ["test@gov.in"], "to": ["test@smb-freight.com"], "date": "2026-06-03T15:30:00",
             "_references": [], "_in_reply_to": ""},
            {"id": "<fwd@smb-freight.com>", "subject": "FW: NOC from MoCA to operate cargo flight at VAOZ airport",
             "from": ["test@smb-freight.com"], "to": ["test1@smb-freight.com"], "date": "2026-06-03T15:36:00",
             "_references": [], "_in_reply_to": ""},
        ]
        resolved = ThreadResolver().resolve(emails)
    ...
    def test_subject_fallback_does_not_merge_unrelated_participants(self):
        ...
        emails = [
            {"id": "<a@x.com>", "subject": "RE: Invoice", "from": ["alice@x.com"], "to": ["bob@x.com"],
             "date": "2026-01-01", "_references": [], "_in_reply_to": ""},
            {"id": "<b@x.com>", "subject": "RE: Invoice", "from": ["carol@y.com"], "to": ["dave@y.com"],
             "date": "2026-01-02", "_references": [], "_in_reply_to": ""},
        ]
        resolved = ThreadResolver().resolve(emails)
    ...
    def test_unresolved_email_self_references(self):
        emails = [
            {"id": "<lonely@x.com>", "subject": "Unique Subject Nobody Replied To",
             "from": ["a@x.com"], "to": ["b@x.com"], "date": None, "_references": [], "_in_reply_to": ""},
        ]
        resolved = ThreadResolver().resolve(emails)
